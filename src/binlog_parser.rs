...
pub struct BinlogParser {
    ...
    pub event_type_inclusion_list: Option<Vec<EventType>>,
}

const MAGIC_VALUE: [u8; 4] = [0xfeu8, 0x62, 0x69, 0x6e];
const BASE_EVENT_TYPES: [EventType; 4] = [
...
impl BinlogParser {
    ...
    pub fn next<S: Read + Seek>(
        ...
        if !BASE_EVENT_TYPES.contains(&event_type) {
            if let Some(ref inclusion_list) = self.event_type_inclusion_list {
                if !inclusion_list.contains(&event_type) {
                    return Ok((header, EventData::NotSupported));
                }
            }
        }
