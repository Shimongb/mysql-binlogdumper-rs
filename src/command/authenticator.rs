...
impl Authenticator {
    ...
    pub async fn connect(&mut self) -> Result<PacketChannel, BinlogError> {
        ...
        let (greeting_buf, sequence) = channel.read_with_sequence().await?;
    ...
    async fn authenticate(
        ...
        let (auth_res, sequence) = channel.read_with_sequence().await?;
    ...
    async fn handle_auth_plugin_switch(
        ...
        let (encrypted_auth_res, sequence) = channel.read_with_sequence().await?;
    ...
    async fn sha2_rsa_authenticate(
        ...
        let (rsa_res, sequence) = channel.read_with_sequence().await?;
        match rsa_res[0] {
            0x01 => {
                ...
                let (auth_res, _) = channel.read_with_sequence().await?;
