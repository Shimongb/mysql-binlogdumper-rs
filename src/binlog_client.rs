...
impl BinlogClient {
    pub async fn connect(&mut self) -> Result<BinlogStream, BinlogError> {
        // Set event type inclusion list
        let event_types_inclusion_list = if self.include_event_types.is_empty() {
            None
        } else {
            EventType::get_event_types_from_str(&self.include_event_types)
        };
        ...
        let parser = BinlogParser {
            ...
            event_type_inclusion_list: event_types_inclusion_list,
        };
